...
export function scanJavaScript(root: string): Finding[] {
  ...
  const sourceFiles = readdirSync(root, { recursive: true }).filter(
    (entry): entry is string => typeof entry === 'string' && /\.(ts|tsx|js|jsx|mjs|cjs)$/.test(entry) && !entry.includes('node_modules'),
  )
    ...
    if (!source.includes(packageName))
      findings.push(
        dependencyFinding({
          ...
          dependency,
          type: 'unused-direct',
          evidence: 'dependency name is absent from tracked source imports; confirm Knip before removal',
  ...
  if (toolFailed)
    findings.push(
      dependencyFinding({
        ...
        dependency: 'knip',
        type: 'tool-missing',
        evidence: 'Knip did not complete successfully through the detected package manager',
        risk: 'blocking',
