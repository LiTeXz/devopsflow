  ...
  if (!packageText)
    return [
      dependencyFinding({
        ...
        type: "missing",
        evidence: "no package manifest found",
        risk: "suggestion",
        action: "skip JavaScript scan",
        autoFixable: false,
      }),
    ];
  const manifest = JSON.parse(packageText) as Record<
    string,
    ...
    if (!source.includes(packageName))
      findings.push(
        dependencyFinding({
          ...
          type: "unused-direct",
          evidence:
            "dependency name is absent from tracked source imports; confirm Knip before removal",
          action: "run Knip and remove only if its result is definite",
          autoFixable: true,
        }),
      );
  }
...
async function main(): Promise<number> {
  ...
  const knip = runTool(
    manager === "bun" ? "bunx" : manager,
    ["exec", "knip", "--no-progress"],
    options.root,
  );
  ...
  if (toolFailed)
    findings.push(
      dependencyFinding({
        ...
        type: "tool-missing",
        evidence:
          "Knip did not complete successfully through the detected package manager",
        risk: "blocking",
        action: "install or invoke the repository-approved Knip version",
        autoFixable: false,
      }),
    );
  if (options.fix && !options.dryRun)
    console.error(
      "--fix requires Knip confirmation; this checker reports candidates without editing manifests",
    );
