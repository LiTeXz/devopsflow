...
export function scanGradle(root: string): Finding[] {
  const wrapper =
    existsSync(join(root, "gradlew.bat")) || existsSync(join(root, "gradlew"));
  if (!wrapper) return [];
  const files = [
    "build.gradle",
    "build.gradle.kts",
    ...
    if (text.includes('implementation("'))
      findings.push(
        dependencyFinding({
          ...
          type: "review",
          evidence:
            "direct declarations require dependencyInsight and source-set confirmation",
          risk: "suggestion",
          action:
            "run wrapper dependencyInsight before changing the declaration",
          autoFixable: false,
        }),
      );
  }
...
async function main(): Promise<number> {
  ...
  const wrapper = existsSync(join(options.root, "gradlew.bat"))
    ? "gradlew.bat"
    : "gradlew";
  ...
  if (options.fix && !options.dryRun)
    console.error(
      "Gradle --fix is gated: verify each candidate with dependencyInsight and the project test task before editing",
    );
