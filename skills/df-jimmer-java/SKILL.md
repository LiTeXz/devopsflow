---
name: df-jimmer-java
description: "Java 专用 Jimmer ORM guardrail。当 Codex 修改 Java Jimmer SQL 实体、APT/Maven/Gradle annotation processing、Spring Boot 集成、生成的 table class query、fetcher、DTO language 文件、graph save、repository 或持久化测试时使用。Java source set 和 Java-first 混合 JVM 项目使用此 skill；Kotlin source set 使用 df-jimmer-kotlin。"
---

# Jimmer Java

处理使用 Jimmer SQL 的 Java 代码时使用此 skill。应将 Jimmer 视为一种围绕不可变实体接口、APT 生成代码、Java DSL 和类型安全 query 构建的编译期 ORM。不要像处理使用可变 POJO 的 JPA 那样处理它。

## Pre-Change Checks

- 修改依赖前先检查现有构建。优先沿用项目当前的 Jimmer 版本和 annotation processing 配置。
- 在 Gradle 或 Maven Java 项目中，应使用 `df-jimmer-sql` 加 annotation processor wiring，而不是 Kotlin KSP。
- 编写 query 前先定位生成源码目录和现有 table class 命名。不要虚构生成的类型名称。
- 识别项目使用的是 Spring Boot auto-configuration、Jimmer repository、直接注入 `JSqlClient`，还是自定义 persistence adapter。

## Entity Rules

- 按本地约定对 Jimmer Java 实体建模，通常使用不可变 Java 接口。
...
- 修改实体、DTO 文件或 mapping 后，运行能够刷新 APT 生成源码的最小构建任务。

## Query Rules

- 优先使用 Java DSL 和生成的 table class 实现类型安全 SQL。将动态 predicate 保留在 DSL 中，不要拼接 SQL 字符串。
...
- 不要在 Java source set 中编写 Kotlin DSL 形式。遵循 Java 示例和本地 generated class 风格。
...
- 修改实体或 DTO 后，运行针对性的 Java/JVM 测试和 compile/APT 任务，确认生成代码已刷新。
- 完成前，在已修改 Java 代码中搜索意外引入的 `EntityManager`、可变实体 setter，或不符合本地约定的 `jakarta.persistence` 用法。
