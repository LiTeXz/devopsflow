...
describe('line catalogs', () => {
  ...
  it('returns blocking CLI exit codes for invalid catalogs', () => {
    ...
    try {
      ...
      const valid = Bun.spawnSync([process.execPath, scriptPath, '--kind', 'permission', '--input', validPath])
      const invalid = Bun.spawnSync([process.execPath, scriptPath, '--kind', 'permission', '--input', invalidPath])

      expect(valid.exitCode).toBe(0)
      expect(invalid.exitCode).toBe(1)
      expect(invalid.stderr.toString()).toContain('legacy colon-delimited identifiers are forbidden')
    } finally {
