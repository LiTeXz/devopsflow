...
describe("TDD Workflow Instructions", () => {
  ...
  it("formats every TypeScript definition as a documented standalone block", () => {
    ...
    expect(skill).not.toContain(
      "用以下 TypeScript 契约描述当前 TDD 状态，不要只用 RED、GREEN 等口述标签：",
    );
    expect(skill).not.toContain(
      "每次向用户报告当前状态时，输出符合此契约的 `typescript` code block",
    );
    expect(blocks.length).toBeGreaterThan(0);

    for (const block of blocks) {
      const definitions = block.match(
        /^(?:type|interface|const|declare const)\s+/gm,
      );
