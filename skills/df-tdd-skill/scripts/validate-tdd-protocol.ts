...
const TASK_TYPES = new Set(['greenfield_feature', 'bug_fix', 'pure_refactor', 'characterize_then_fix'])
...
function requireFields(block: TddBlock, fields: string[], prefix: string, errors: string[]): void {
...
function validateStart(start: TddBlock | null): string[] {
  ...
  requireFields(
    start,
    ['task_type', 'protected_behavior', 'stable_boundary', 'first_test_to_write', 'expected_red_reason', 'current_contract_wrong', 'wrong_contract_plan'],
    'tdd_start',
    errors,
  )
  ...
  if (!TASK_TYPES.has(taskType)) {
    errors.push(`tdd_start.task_type must be one of ${[...TASK_TYPES].sort().join(', ')}`)
  }

  const plan = start.wrong_contract_plan as string
  if (!WRONG_CONTRACT_PLANS.has(plan)) {
    errors.push(`tdd_start.wrong_contract_plan must be one of ${[...WRONG_CONTRACT_PLANS].sort().join(', ')}`)
  }

  if (start.current_contract_wrong === true && plan === 'none') {
    errors.push('tdd_start.wrong_contract_plan cannot be none when current_contract_wrong is true')
  }

  if (taskType === 'characterize_then_fix' && plan !== 'fix_after_characterization') {
    errors.push('characterize_then_fix requires wrong_contract_plan: fix_after_characterization')
  }

  return errors
}

function validateStates(states: TddBlock[], requireRed: boolean = false): string[] {
  const errors: string[] = []
  ...
  for (let index = 0; index < states.length; index++) {
    ...
    if (!PHASES.has(phase)) {
      errors.push(`tdd_state[${idx}].phase must be one of ${[...PHASES].sort().join(', ')}`)
      continue
    }
    if (isBlank(evidence)) {
    ...
    const missingPrevious = PHASE_ORDER.slice(0, phasePosition).filter((p) => !observed.includes(p))
    if (missingPrevious.length) {
      errors.push(`tdd_state[${idx}].phase ${phase} is out of order; missing ${JSON.stringify(missingPrevious)}`)
    }
    ...
    if (phase === 'red_observed') {
      ...
      const tokens = ['fail', 'red', '失败', '红', 'expected', '预期', 'reason', '原因']
      if (!tokens.some((t) => lowered.includes(t))) {
        errors.push('red_observed evidence should explain the failure reason and target risk')
      }
      ...
      if (isBlank(state.command)) {
        errors.push('red_observed command is required, or use command: none with explanation in evidence')
      }
    }
    if (phase === 'green_reached') {
      if (state.exit_code !== undefined && state.exit_code !== 0 && state.exit_code !== '0' && state.exit_code !== null) {
        errors.push('green_reached exit_code should be 0 when recorded')
      }
      if (isBlank(state.command)) {
        errors.push('green_reached command is required, or use command: none with explanation in evidence')
      }
    }
  }
  ...
  testsRun.forEach((item, index) => {
    ...
    if (item && typeof item === 'object' && !Array.isArray(item)) {
      ...
      if (!phase || !['red', 'green', 'final'].includes(phase)) {
        errors.push(`tdd_finish.tests_run[${idx}].phase must be red, green, or final`)
      }
      if (isBlank(command)) {
      ...
      if (phase === 'red' && (exitCode === 0 || exitCode === '0')) {
        errors.push(`tdd_finish.tests_run[${idx}].exit_code should be non-zero for red`)
      }
      if (['green', 'final'].includes(phase as string) && exitCode !== undefined && exitCode !== 0 && exitCode !== '0' && exitCode !== null) {
        errors.push(`tdd_finish.tests_run[${idx}].exit_code should be 0 for green/final`)
      }
    } else {
      const lowered = String(item).toLowerCase()
      if (!phase) {
        errors.push(`tdd_finish.tests_run[${idx}] should include a red/green/final phase hint or use a structured entry`)
      }
      const cmdTokens = ['test', 'pytest', 'cargo', 'mvn', 'gradle', 'npm', 'pnpm', 'go test', 'passed', 'failed', '通过', '失败']
      if (!cmdTokens.some((t) => lowered.includes(t))) {
        errors.push(`tdd_finish.tests_run[${idx}] should include a command, test name, or concrete result`)
      }
    }
  })
...
function validateFinish(finish: TddBlock | null): string[] {
  ...
  if (!TASK_TYPES.has(taskType)) {
    errors.push(`tdd_finish.task_type must be one of ${[...TASK_TYPES].sort().join(', ')}`)
  }
  ...
  const { errors: testErrors, phases: testPhases } = validateTestsRun(finish.tests_run)
  ...
  if (finish.green_reached === true && !(testPhases.has('green') || testPhases.has('final'))) {
    errors.push('tdd_finish.tests_run should include green or final phase evidence')
  }

  if (finish.current_contract_wrong === true && finish.wrong_contract_characterized !== true) {
    errors.push('wrong current contracts must be characterized before completion')
  }

  if (taskType === 'characterize_then_fix' && finish.wrong_contract_fixed !== true) {
    errors.push('characterize_then_fix requires tdd_finish.wrong_contract_fixed: true, or the task is not complete')
  }
...
function main(): number {
  ...
  const scriptIdx = rawArgs.findIndex((a) => a.includes('validate-tdd-protocol'))
  ...
  if (!['before_edit', 'state', 'finish'].includes(stage)) {
    console.error('Usage: validate-tdd-protocol.ts --stage <stage> [--input <file>]')
    return 1
  }
...
if (import.meta.main) {
  process.exit(runLoggedScript({ scriptName: 'validate-tdd-protocol' }, () => main()))
}
