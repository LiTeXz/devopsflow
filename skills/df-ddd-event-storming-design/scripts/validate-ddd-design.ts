...
const SMELLS: Record<string, string[]> = {
  ...
  actorless_design: [
    "[\\s\\S]*(命令清单|#\\s*命令|##\\s*命令|#{1,6}\\s*(Command Catalog|Commands))(?![\\s\\S]*(Actor|主体|发起|角色|外部系统|Timer|System))",
  ],
  flat_admin_nouns: [
...
function findConfirmationGateViolations(text: string): string[] {
  ...
  const hasDownstreamSections =
    hasAny(text, [
      "领域事件清单|#\\s*领域事件|##\\s*领域事件|#{1,6}\\s*(Domain Event Catalog|Domain Events)",
    ]) &&
    hasAny(text, [
      "命令清单|#\\s*命令|##\\s*命令|#{1,6}\\s*(Command Catalog|Commands)",
    ]) &&
    hasAny(text, [
      "聚合设计|#\\s*聚合|##\\s*聚合|#{1,6}\\s*(Aggregate Design|Aggregates)",
    ]);

  const hasReadModelSection = hasAny(text, [
    "读模型设计|#\\s*读模型|##\\s*读模型|#{1,6}\\s*(Read Model Design|Read Models)",
  ]);
