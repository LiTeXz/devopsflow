...
export function scanMaven(root: string): Finding[] {
  ...
  const duplicateTags =
    (text.match(/<groupId>/g) ?? []).length !==
    new Set(text.match(/<artifactId>([^<]+)/g) ?? []).size;
  return duplicateTags
    ? [
        dependencyFinding({
          ...
          evidence:
            "repeated artifact declarations require dependency:tree and effective-pom confirmation",
          action:
            "run mvn dependency:analyze and inspect profiles before editing",
          autoFixable: false,
        }),
      ]
    : [];
...
async function main(): Promise<number> {
  ...
  const wrapper = existsSync(join(options.root, "mvnw.cmd"))
    ? "mvnw.cmd"
    : existsSync(join(options.root, "mvnw"))
      ? "mvnw"
      : "mvn";
  ...
  if (options.fix && !options.dryRun)
    console.error(
      "Maven --fix is gated: verify dependency:analyze and mvn verify before editing",
    );
