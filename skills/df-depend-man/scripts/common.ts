...
export function emit(findings: Finding[], format: Options["format"]): number {
  if (format === "json") console.log(JSON.stringify(findings, null, 2));
    ...
    for (const finding of findings)
      console.log(
        `${finding.risk.toUpperCase()} ${finding.ecosystem} ${finding.file}: ${finding.dependency} - ${finding.evidence}. ${finding.action}`,
      );
...
export function dependencyFinding(
  input: Omit<Finding, "risk"> & { risk?: Finding["risk"] },
): Finding {
...
export function runTool(
  ...
  args: string[],
  cwd: string,
): { available: boolean; exitCode: number } {
  try {
