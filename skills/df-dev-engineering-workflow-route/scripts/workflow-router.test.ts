...
describe("routeTask", () => {
  ...
  test("emits English reasons for every registered skill", () => {
    const decisions = Object.values(TaskType).flatMap((taskType) =>
      Object.values(RiskDimension).map((risk) =>
        routeTask({ taskType, risks: [risk] }),
      ),
    );
    const reasons = decisions.flatMap((decision) =>
      decision.requiredSkills.map(({ reason }) => reason),
    );
  ...
  test("routes a bug fix through debugging, TDD, and verification", () => {
    expect(routeTask({ taskType: TaskType.BugFix }).executionOrder).toEqual([
  ...
  test("adds handoff when domain discovery precedes implementation", () => {
    ...
    expect(new Set(decision.executionOrder).size).toBe(
      decision.executionOrder.length,
    );
  });
