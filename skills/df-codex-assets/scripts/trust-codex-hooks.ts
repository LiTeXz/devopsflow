...
function pluginRelativePath(pluginRoot: string, configuredPath: string): string {
...
function normalizedCommandHook(eventKey: CodexEventKey, handler: CommandHook, platform: NodeJS.Platform): Record<string, unknown> | undefined {
...
function commandHookHash(eventKey: CodexEventKey, group: MatcherGroup, handler: CommandHook, platform: NodeJS.Platform): string | undefined {
...
function configuredHookStates(parsed: Record<string, unknown>): Record<string, unknown> {
...
function updateExistingSection(content: string, key: string, trustedHash: string): string {
  ...
  const hashIndex = lines.findIndex((line, index) => index > sectionStart && index < sectionEnd && /^\s*trusted_hash\s*=/.test(line))
...
function appendStateSection(content: string, key: string, trustedHash: string): string {
  const newline = content.includes('\r\n') ? '\r\n' : '\n'
  let updated = content
...
function writeConfigAtomically(configPath: string, originalContent: string | undefined, updatedContent: string): void {
  const configDirectory = dirname(configPath)
  mkdirSync(configDirectory, { recursive: true })
  const temporaryPath = join(configDirectory, `.${configPath.split(/[\\/]/).at(-1)}.${process.pid}.${randomUUID()}.tmp`)
  try {
...
if (import.meta.main) {
  process.exit(runLoggedScript({ scriptName: 'trust-codex-hooks' }, () => runCli()))
}
