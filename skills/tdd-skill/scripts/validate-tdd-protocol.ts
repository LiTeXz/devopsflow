...
function validateTestsRun(testsRun: unknown): { errors: string[]; phases: Set<string> } {
  const errors: string[] = []
  const phases = new Set<string>()

  if (!Array.isArray(testsRun) || !testsRun.length) {
    return { errors: ["tdd_finish.tests_run must be a non-empty list"], phases }
  }
