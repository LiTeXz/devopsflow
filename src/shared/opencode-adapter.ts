...
export function isOpenCodeDfOpsVcsManager(input: OpenCodeToolBeforeInput): boolean {
  const name =
...
export function shouldBlockOpenCodeToolInput(input: OpenCodeToolBeforeInput): BlockDecision | undefined {
  ...
  return shouldBlockOpenCodeTool(toolName, toolInput, isOpenCodeSubagent(input), cwd, isOpenCodeDfOpsVcsManager(input))
}
