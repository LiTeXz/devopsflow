...
export function hasShellRedirection(tokens: string[]): boolean {
  ...
  return tokens.some((token) => redirectOps.has(token) || token.startsWith('>') || token.startsWith('>>'))
}
...
export function pythonWriteReason(args: string[]): string | undefined {
  ...
  if (filtered[0] === '-c' || filtered[0].startsWith('-c')) {
    const code = filtered[0].startsWith('-c') && filtered[0].length > 2 ? `${filtered[0].slice(2)} ${filtered.slice(1).join(' ')}` : filtered.slice(1).join(' ')
...
export const SHELL_BINARIES = new Set(['bash', 'sh', 'zsh', 'dash', 'ksh', 'fish', 'csh', 'tcsh'])
