...
const WRITABLE_AGENT_TOKENS = ['worker', 'subagent', 'fork', 'background'] as const

export const SESSION_HOOK_NAMES = new Set(['SessionStart', 'session_start', 'sessionStart'])
export const SUBAGENT_START_EVENTS = new Set(['SubagentStart', 'subagent_start', 'subagentStart'])
export const SUBAGENT_STOP_EVENTS = new Set(['SubagentStop', 'subagent_stop', 'subagentStop'])
export const PRE_TOOL_USE_EVENTS = new Set(['PreToolUse', 'pre_tool_use', 'preToolUse', ''])

export const SHELL_TOOL_NAMES = new Set(['Bash', 'shell', 'exec', 'exec_command', 'shell_command', 'unified_exec'])
export const DIRECT_WRITE_TOOL_NAMES = new Set(['Write', 'Edit', 'MultiEdit', 'NotebookEdit', 'apply_patch'])
export const SUBAGENT_TOOL_NAMES = new Set(['Task', 'delegate_task', 'SubagentStart', 'subagent_start', 'WorkerStart', 'worker_start'])
...
export function findToolInput(payload: Payload): ToolInput | null {
  ...
  return value && typeof value === 'object' && !Array.isArray(value) ? (value as ToolInput) : null
}
...
export function findAgentName(payload: Payload): string {
  const values = [payload.agent_type, payload.agentType, payload.agentName, payload.agentDisplayName]
  return values
...
function collectAgentContextValues(payload: Payload): string[] {
  ...
  if (agent) {
    values.push(stringValue(agent.mode), stringValue(agent.name), stringValue(agent.type))
  }
  ...
  if (session) {
    ...
    if (sessionAgent) {
      values.push(stringValue(sessionAgent.mode), stringValue(sessionAgent.name))
    }
  }
