...
export function shouldBlockTool(toolName: string, toolInput: ToolInput, cwd: string, isDfPublisher = false): BlockDecision | undefined {
...
function proxyEscalationDecision(tokens: string[]): BlockDecision | undefined {
  const { stripLauncherPrefix } = require('@/shared/command-parser')
...
function analyzeGitWrite(tokens: string[], cwd: string): BlockDecision | undefined {
  ...
  if (subcommand && GIT_WRITE_SUBCOMMANDS.has(subcommand)) {
    ...
    return blockCurrentBranchWrite(effectiveCwd, `\`git ${subcommand}\` 会修改工作区、索引或提交历史`)
  }
  return undefined
}
...
function main(payload: Payload | null = readPayload()): number {
  ...
  const isDfOpsVcsManager = sessionId ? isDfOpsVcsManagerSession(sessionId) : false
...
if (import.meta.main) {
  ...
  process.exit(runLoggedScript({ payload, scriptName: 'prevent-protected-branch-push' }, () => main(payload)))
}
