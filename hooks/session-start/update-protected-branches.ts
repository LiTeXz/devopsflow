...
export const PROTECTED_BRANCHES = ['dev', 'main', 'master', 'develop', 'devlop'] as const
...
export function updateProtectedBranches(cwd: string): UpdateResult {
  ...
  const remoteHeads = requireGit(
    cwd,
    ['ls-remote', '--heads', 'origin', ...PROTECTED_BRANCHES.map((branch) => `refs/heads/${branch}`)],
    '无法读取 origin 的受保护分支',
  ).stdout
  const availableBranches = PROTECTED_BRANCHES.filter((branch) => remoteHeads.split(/\r?\n/).some((line) => line.endsWith(`\trefs/heads/${branch}`)))
  if (availableBranches.length === 0) return { updated: [], skipped: false }

  requireGit(
    cwd,
    ['fetch', '--prune', 'origin', ...availableBranches.map((branch) => `+refs/heads/${branch}:refs/remotes/origin/${branch}`)],
    '无法从 origin 获取受保护分支',
  )
  ...
  for (const branch of availableBranches) {
    ...
    if (git(cwd, ['show-ref', '--verify', '--quiet', remoteRef]).exitCode !== 0) {
      continue
    }

    const localRef = `refs/heads/${branch}`
    const remoteOid = requireGit(cwd, ['rev-parse', remoteRef], `无法解析 origin/${branch}`).stdout
    ...
    if (localOidResult.exitCode === 0) {
      ...
      const ancestor = git(cwd, ['merge-base', '--is-ancestor', localOidResult.stdout, remoteOid])
      if (ancestor.exitCode !== 0) {
        throw new Error(`受保护分支 ${branch} 无法 fast-forward 到 origin/${branch}；本地与远端可能存在冲突`)
      }
      if (checkedOutBranch === branch) {
        requireGit(cwd, ['merge', '--ff-only', remoteRef], `更新当前分支 ${branch} 失败`)
      } else {
        requireGit(cwd, ['update-ref', localRef, remoteOid, localOidResult.stdout], `更新 ${branch} 失败`)
      }
    } else {
      requireGit(cwd, ['update-ref', localRef, remoteOid], `创建本地分支 ${branch} 失败`)
    }
    updated.push(branch)
  }
...
export function writeFailure(error: unknown): void {
  ...
  const instruction = 'Agent 必须立即检查 Git 状态和分支差异，并与用户协商如何处理冲突；不得强推、重置或擅自覆盖本地提交。'
...
if (import.meta.main) {
  ...
  process.exit(runLoggedScript({ payload, scriptName: 'update-protected-branches' }, () => main(payload)))
}
