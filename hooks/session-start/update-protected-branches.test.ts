...
import { PROTECTED_BRANCHES, updateProtectedBranches, writeFailure } from './update-protected-branches'
...
describe('SessionStart protected branch updater', () => {
  ...
  it('fast-forwards every protected local branch that exists on origin', async () => {
    ...
    expect((await Bun.file(join(workspace, 'remote.txt')).text()).trim()).toBe('remote')
  })
  ...
  it('fails instead of overwriting a diverged protected branch', () => {
    ...
    expect(git(workspace, 'log', '-1', '--format=%s', 'main')).toBe('local update')
  })
  ...
  it('injects immediate conflict-handling instructions for the agent', () => {
    ...
    expect(output.hookSpecificOutput?.additionalContext).toContain('Agent 必须立即检查')
    expect(output.hookSpecificOutput?.additionalContext).toContain('与用户协商如何处理冲突')
  })
})
