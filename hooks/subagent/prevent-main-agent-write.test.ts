...
describe('MainAgentWriteGuard', () => {
  ...
  describe('shell command blocking', () => {
    it('blocks main agent shell redirection, rm, git add, and package mutations', () => {
      ...
      expect(shouldBlockTool('Bash', { command: 'bun add zod' })).not.toBeNull()
      expect(shouldBlockTool('Bash', { command: 'bun run generate' })).not.toBeNull()
      expect(shouldBlockTool('Bash', { command: 'bun run build' })).not.toBeNull()
    })
  })
  ...
  describe('safe commands', () => {
    it('allows main agent read, status, and test commands', () => {
      ...
      expect(shouldBlockTool('Bash', { command: 'rg -n DevopsFlow README.md' })).toBeUndefined()
      ...
      expect(shouldBlockTool('Bash', { command: 'bun run test' })).toBeUndefined()
      expect(shouldBlockTool('Bash', { command: 'bun run typecheck' })).toBeUndefined()
  ...
  describe('protected branch write for registered Codex worker', () => {
    ...
    it('uses git -C cwd instead of parent cwd for registered worker git writes', () => {
      ...
      expect(shouldBlockTool('Bash', { command: `git -C "${allowedNested}" add README.md` }, 'worker-1', root)).toBeUndefined()

      const decision = shouldBlockTool('Bash', { command: `git -C "${blockedNested}" add README.md` }, 'worker-1', root)
      expect(decision).not.toBeNull()
      expect(decision?.reason).toInclude('cwd')
