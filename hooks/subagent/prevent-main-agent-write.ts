...
import { isDfOpsVcsManagerSession, isRegisteredSubagentSession, loadState, saveState } from '@/shared/state-store'
...
export function shouldBlockTool(
  ...
  const isRegisteredSession = sessionId ? isRegisteredSubagentSession(sessionId) : false
  if (!isRegisteredSession && !hasWritableAgentContext) {
    const sessionReason = sessionId ? `当前 session \`${sessionId}\` 未登记` : 'payload 缺少 session_id'
    return createBlockDecision('unknown', `${decision.reason}，${sessionReason}且 payload 未显示受支持的 Codex worker/fork/background 上下文`)
  }
...
export function shouldBlockOpenCodeTool(
  ...
  if (ocCommand && containsBlockedGitGh(ocCommand)) {
    if (isDfOpsVcsManager) {
      if (cwd) {
        const pbDecision = protectedBranchWriteDecision(cwd, 'df-ops-vcs-manager 在保护分支上执行 git/gh 操作')
        if (pbDecision) return pbDecision
      }
      return undefined
    }
    return createBlockDecision('unknown', 'git push/commit/gh issue/gh pr 操作已被禁止；请委托 df-ops-vcs-manager Codex worker session 完成')
  }
  const decision = decisionForTool(toolName, toolInput, cwd)
  if (!decision) return undefined
  if (decision.escalation) return decision
  ...
  if (!isSubagent) {
    return createBlockDecision('unknown', `${decision.reason}，当前 OpenCode agent 不是兼容适配器识别的写入会话，main Codex session 禁止写入`)
  }
...
function decisionForTool(toolName: string, toolInput: ToolInput, cwd?: string): ToolDecision | undefined {
...
function decisionForCommand(command: string, cwd?: string): ToolDecision | undefined {
...
function gitWriteReason(tokens: string[], cwd?: string): { reason: string; cwd?: string } | undefined {
  const subcommand = gitSubcommand(tokens.slice(1))
...
function main(payload: Payload | null = readPayload()): number {
  ...
  const decision = shouldBlockTool(toolName, toolInput, sessionId, cwd, isWritableAgentContext(payload))
...
if (import.meta.main) {
  ...
  process.exit(runLoggedScript({ payload, scriptName: 'prevent-main-agent-write' }, () => main(payload)))
}
