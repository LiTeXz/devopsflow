...
`PostToolUse` 在成功的 `apply_patch` 完成后解析 patch 中新增或更新的文件；matcher 同时兼容官方 `Edit`/`Write` alias，而 hook input 仍使用 canonical `tool_name: "apply_patch"`。TypeScript/JSON 等文件在当前 Git repository 声明 `biome.json` 或 `biome.jsonc` 时运行本地 Biome `format --write`；`.java`、`.kt`、`.gradle` 和 `.gradle.kts` 分别路由到 `spotlessJavaApply`、`spotlessKotlinApply` 或 `spotlessGradleApply`。每个 Spotless 文件从自身所在目录向上查找 `gradlew.bat`（Windows）或 `gradlew`，搜索最多到本次 hook 的 `cwd`，不会越过该边界，并始终传入 `--no-daemon`。删除、缺失、绝对路径和 repository 外路径会被跳过；formatter 不可用或失败时 hook 不会隐式安装依赖，并保持 fail-open，通过简短 `systemMessage` 报告原因。
