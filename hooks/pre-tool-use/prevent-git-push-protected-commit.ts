...
export function shouldBlockGitMutation(
  command: string,
  cwd: string,
): GitMutationDecision | undefined {
...
function writeBlockMessage(decision: GitMutationDecision): void {
  if (decision.kind === "push") {
    console.error(
      "DevopsFlow 已阻止 Agent 执行 push；任何分支都必须由用户人工推送。",
    );
  } else {
    console.error(
      `DevopsFlow 已阻止 Agent 在受保护分支 ${decision.branch} 上执行 commit。`,
    );
  }
  console.error(
    "请提醒用户先审查当前代码，再由用户逐个手动提交；不要把 commit 或 push 委托给 Agent。",
  );
}
...
function main(payload: Payload | null = readPayload()): number {
  ...
  if (!command) return 0;
  const decision = shouldBlockGitMutation(
    command,
    findWorkdir(payload, toolInput),
  );
...
if (import.meta.main) {
  ...
  process.exit(
    runLoggedScript(
      { payload, scriptName: "prevent-git-push-protected-commit" },
      () => main(payload),
    ),
  );
}
