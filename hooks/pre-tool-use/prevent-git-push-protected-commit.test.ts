...
describe("PreToolUse Git mutation guard", () => {
  it("blocks every push regardless of destination or current branch", () => {
    ...
    expect(
      shouldBlockGitMutation("git push origin feature/demo", cwd)?.kind,
    ).toBe("push");
    expect(shouldBlockGitMutation("git push origin HEAD:main", cwd)?.kind).toBe(
      "push",
    );
    expect(
      shouldBlockGitMutation("env X=1 git push --all origin", cwd)?.kind,
    ).toBe("push");
    expect(
      shouldBlockGitMutation('bash -c "git push origin feature/demo"', cwd)
        ?.kind,
    ).toBe("push");
  });
  ...
  it("uses git -C when determining the commit target branch", () => {
    ...
    expect(
      shouldBlockGitMutation(`git -C "${protectedRepo}" commit -m test`, root),
    ).toMatchObject({ branch: "master", kind: "commit" });
  });
  ...
  it("returns exit code 2 and tells the user to review and commit manually", () => {
    ...
    const result = Bun.spawnSync({
      cmd: [
        process.execPath,
        join(import.meta.dir, "prevent-git-push-protected-commit.ts"),
      ],
      cwd,
