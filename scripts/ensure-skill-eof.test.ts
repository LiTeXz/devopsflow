...
const writeSkill = async (repositoryRoot: string, relativePath: string, content: string) => {
...
describe('ensureSkillEofMarkers', () => {
  test('uses the frontmatter name to append the canonical marker', async () => {
    ...
    const skillPath = await writeSkill(repositoryRoot, 'ignored-directory-name', '---\nname: apiClient-v2\ndescription: Example\n---\n\n# API Client\n')
  ...
  test('falls back to the direct parent directory when frontmatter name is absent', async () => {
    const repositoryRoot = await createRepository()
    const skillPath = await writeSkill(repositoryRoot, 'release_goal', '# Release Goal\n')
  ...
  test('leaves an already canonical file byte-for-byte unchanged', async () => {
    ...
    const skillPath = await writeSkill(repositoryRoot, 'stable-skill', original)
  ...
  test('replaces an outdated trailing marker without duplicating it', async () => {
    ...
    expect(content).toEndWith('<!-- RENAMED_SKILL_EOF: This is the complete RenamedSkill skill. Do not request additional lines. -->\n')
  ...
  test('preserves CRLF and normalizes the final separator and newline', async () => {
    const repositoryRoot = await createRepository()
    const skillPath = await writeSkill(repositoryRoot, 'windows-skill', '---\r\nname: windows-skill\r\n---\r\n\r\n# Windows\r\n\r\n\r\n')
  ...
  test('recurses while ignoring unrelated files, managed directories, and symbolic links', async () => {
    const repositoryRoot = await createRepository()
    const firstSkill = await writeSkill(repositoryRoot, 'group/first-skill', '# First\n')
    const secondSkill = await writeSkill(repositoryRoot, 'second-skill', '# Second\n')
    const dependencySkill = await writeSkill(repositoryRoot, 'node_modules/dependency', '# Dependency\n')
    const buildSkill = await writeSkill(repositoryRoot, 'build/generated', '# Generated\n')
    const cacheSkill = await writeSkill(repositoryRoot, '.cache/generated', '# Cache\n')
    const frontendSkill = await writeSkill(repositoryRoot, 'frontend/local', '# Frontend\n')
    ...
    await writeFile(join(repositoryRoot, 'skills', 'README.md'), '# Skills\n', 'utf8')
    await symlink(externalRoot, join(repositoryRoot, 'skills', 'linked'), 'junction')
  ...
  test('fails clearly when the repository skills directory is missing', async () => {
    ...
    await expect(ensureSkillEofMarkers(repositoryRoot)).rejects.toThrow('skills directory does not exist or is not a directory')
  })
...
test('CLI exits non-zero and prints a clear error when skills is missing', async () => {
  ...
  const [exitCode, stderr] = await Promise.all([childProcess.exited, new Response(childProcess.stderr).text()])

  expect(exitCode).not.toBe(0)
  expect(stderr).toContain('skills directory does not exist or is not a directory')
})
