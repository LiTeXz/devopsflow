  ...
  for (const [index, rawLine] of readFileSync(path, "utf-8")
    ...
    if (fence) {
      if (ignoredFence) {
        ...
        ignoredFence = !["", "markdown", "text", "yaml", "yml"].includes(
          language,
        );
      }
      continue;
...
describe("prompt language", () => {
    ...
    const violations = promptFiles(SKILLS_ROOT)
      ...
      .flatMap((path) => {
        ...
        const references = content.matchAll(
          /`((?:\.\.?\/)?[^`\s<>*]+\/[^`\s<>*]+\.(?:ts|md|yaml|yml|json|toml|svg|kts))`/g,
        );

        return [...references].flatMap((match) => {
          const reference = match[1];
          if (
            reference.startsWith(".codex/") ||
            reference.startsWith(".devopsflow/")
          ) {
            return [];
          }
          const targets = [
          ...
          return targets.some((target) => existsSync(target))
            ? [`${relative(ROOT, path).replaceAll("\\", "/")}: ${reference}`]
            : [];
        });
      });
  ...
  it("keeps file links resolvable and filename-labeled", () => {
    const violations = promptFiles(SKILLS_ROOT)
      .filter((path) => extname(path) === ".md")
      .flatMap((path) => {
        const links = readFileSync(path, "utf-8").matchAll(
          /\[([^\]]+)\]\(([^)<]+)\)/g,
        );
        return [...links].flatMap((match) => {
          ...
          return existsSync(resolved) && label === basename(target)
            ? []
            : [
                `${relative(ROOT, path).replaceAll("\\", "/")}: [${label}](${target})`,
              ];
        });
      });
  ...
  it("uses Chinese for prose while allowing titles and terminology", () => {
    const violations = promptFiles(SKILLS_ROOT).flatMap((path) =>
      englishProseLines(path).map(
        (line) => `${relative(ROOT, path).replaceAll("\\", "/")}:${line}`,
      ),
    );
