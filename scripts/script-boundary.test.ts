...
describe("repository scripts boundary", () => {
  it("exposes every repository script through package.json", () => {
    ...
    const repositoryScripts = readdirSync(SCRIPTS_DIRECTORY).filter(
    ...
    expect(packageJson.scripts?.["test:repository-scripts"]).toBe(
      "bun test scripts/",
    );
  });
  ...
  it("keeps direct implementation paths out of repository consumers", () => {
    ...
    for (const file of collectFiles(ROOT)) {
      ...
      for (const implementationPath of implementationPaths) {
        expect(content, `${projectPath}: ${implementationPath}`).not.toContain(
          implementationPath,
        );
      }
    }
