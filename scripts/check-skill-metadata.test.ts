...
function writeSkill(skillsRoot: string, folderName: string, frontMatterName: string, description = 'Test skill') {
  const skillDir = join(skillsRoot, folderName)
  mkdirSync(skillDir, { recursive: true })
  writeFileSync(join(skillDir, 'SKILL.md'), `---\nname: ${frontMatterName}\ndescription: ${description}\n---\n\n# Test\n`, 'utf-8')
}

function writeOpenAiAgent(skillsRoot: string, folderName: string, displayName: string, shortDescription = 'Test skill') {
...
describe('skill metadata validation', () => {
      ...
      expect(validateSkillMetadata(skillsRoot)).toEqual([
        {
          ...
          message: 'front matter name "different-name" must match folder "df-example-skill"',
        },
      ])
    })
  })
  ...
  it('rejects display names that are not derived from the folder name', () => {
    withSkillsRoot((skillsRoot) => {
      ...
      expect(validateSkillMetadata(skillsRoot)).toEqual([
        {
          path: join(skillsRoot, 'df-example-skill', 'agents', 'openai.yaml'),
          message: 'interface.display_name "Df Example Skill" must be "Example Skill"',
        },
      ])
    })
  })
  ...
  it('rejects short descriptions that do not match SKILL.md descriptions', () => {
    withSkillsRoot((skillsRoot) => {
      writeSkill(skillsRoot, 'df-example-skill', 'df-example-skill', 'Full SKILL description')
      writeOpenAiAgent(skillsRoot, 'df-example-skill', 'Example Skill', 'Short UI description')

      expect(validateSkillMetadata(skillsRoot)).toEqual([
        {
          path: join(skillsRoot, 'df-example-skill', 'agents', 'openai.yaml'),
          message: 'interface.short_description must exactly match SKILL.md description "Full SKILL description"',
        },
      ])
    })
  })
