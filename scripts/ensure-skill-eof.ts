...
const MARKER_PATTERN = /<!--\s+[A-Z0-9_]+_EOF:\s+This is the complete .+? skill\. Do not request additional lines\.\s+-->/
const EXCLUDED_DIRECTORIES = new Set(['.cache', '.git', '.gradle', 'build', 'dist', 'frontend', 'node_modules'])
...
const buildMarker = (name: string) => {
  ...
  const markerName = upperSnakeName.endsWith('_SKILL') ? upperSnakeName : `${upperSnakeName}_SKILL`
...
const ensureMarker = async (skillPath: string) => {
  ...
  const skillName = readFrontmatterName(original) ?? basename(join(skillPath, '..'))
  const marker = buildMarker(skillName)
  const contentWithoutTrailingWhitespace = original.trimEnd()

  if (contentWithoutTrailingWhitespace.endsWith(marker) && original === `${contentWithoutTrailingWhitespace}${newline}`) {
    return false
  }

  const trailingMarkerMatch = contentWithoutTrailingWhitespace.match(new RegExp(`${MARKER_PATTERN.source}$`))
  const body = trailingMarkerMatch ? contentWithoutTrailingWhitespace.slice(0, trailingMarkerMatch.index).trimEnd() : contentWithoutTrailingWhitespace
...
export const ensureSkillEofMarkers = async (repositoryRoot = process.cwd()): Promise<EnsureSkillEofResult> => {
  const skillsRoot = join(repositoryRoot, 'skills')

  try {
    const skillsRootStat = await lstat(skillsRoot)
    if (!skillsRootStat.isDirectory() || skillsRootStat.isSymbolicLink()) throw new Error()
  } catch {
    throw new Error(`skills directory does not exist or is not a directory: ${skillsRoot}`)
  }
