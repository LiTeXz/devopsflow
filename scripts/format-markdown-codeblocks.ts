...
function findFences(lines: string[]): Fence[] {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    const isClosingFence =
      info.trim() === "" &&
      marker[0] === open.markerCharacter &&
      marker.length >= open.markerLength;
...
export function runBiomeFormatter(source: string, language: string): string {
  ...
  if (result.exitCode !== 0) {
    throw new Error(
      `Biome failed for ${language} code block: ${result.stderr.toString().trim()}`,
    );
  }
...
export function formatMarkdownCodeBlocks(markdown: string): string {
  ...
  for (const fence of fences.reverse()) {
    ...
    try {
      ...
      throw new Error(
        `${fence.language} code block at line ${fence.contentStart + 1}: ${detail}`,
      );
    }
    lines.splice(
      fence.contentStart,
...
function main(args: string[] = process.argv.slice(2)): number {
  ...
  if (!files.length) {
    console.error(
      "Usage: format-markdown-codeblocks.ts [--write] <file.md> [...]",
    );
    return 2;
  }
  let changed = false;
...
function checkStagedMarkdown(): number {
  ...
  if (listed.exitCode !== 0) {
    console.error(
      `Unable to list staged Markdown files: ${listed.stderr.toString().trim()}`,
    );
    return 2;
  }
  ...
  for (const path of paths) {
    ...
    if (content.exitCode !== 0) {
      console.error(
        `${path}: unable to read staged content: ${content.stderr.toString().trim()}`,
      );
      return 2;
    }
    let formatted: string;
